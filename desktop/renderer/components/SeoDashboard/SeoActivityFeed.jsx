...
function ExpandedDetails({ changes }) {
  const rows = [];
  if (changes?.focusKeyword?.after) {
    rows.push({ label: 'Keyword', before: changes.focusKeyword.before || '—', after: changes.focusKeyword.after });
  }
  if (changes?.metaTitle?.after) {
    rows.push({ label: 'Meta Title', before: changes.metaTitle.before || '—', after: changes.metaTitle.after });
  }
  if (changes?.metaDescription?.after) {
    rows.push({ label: 'Meta Desc', before: changes.metaDescription.before || '—', after: changes.metaDescription.after });
  }
  return (
    <div className="mt-3 pl-3 border-l-2 border-gray-700 flex flex-col gap-2">
      ...
      {changes?.contentRewritten && (
        <p className="text-xs text-amber-400">Full content rewrite applied</p>
      )}
      {changes?.internalLinksAdded > 0 && (
        <p className="text-xs text-brand-400">{changes.internalLinksAdded} internal link{changes.internalLinksAdded > 1 ? 's' : ''} added</p>
      )}
    </div>
  );
}
...
function FeedEntry({ log }) {
  ...
  return (
    <div
      className="border-b border-gray-700/50 py-3 px-4 cursor-pointer hover:bg-gray-700/20 transition-colors"
