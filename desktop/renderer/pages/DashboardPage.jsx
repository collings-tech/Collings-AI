...
// Main page
// ---------------------------------------------------------------------------
export default function DashboardPage({ onSelectSite, onLogout }) {
  ...
  const loadSeoData = useCallback(async (loadedSites) => {
    ...
    try {
      const overviewResult = await window.electronAPI.invoke('seo:get-overview');
      const overview = overviewResult?.overview || [];

      const trendResults = await Promise.allSettled(
        loadedSites.map((site) =>
          window.electronAPI.invoke('seo:get-score-trend', { siteId: site._id || site.id, days: 7 })
        )
      );
  ...
  const handleDeleteSite = async (id) => {
    try {
      const result = await window.electronAPI.invoke('sites:delete', { id });
      if (result.error) alert(result.error);
      else setSites(sites.filter((s) => (s.id || s._id) !== id));
    } catch (err) {
  ...
  return (
    <div className={`min-h-screen flex flex-col ${isDark ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'}`}>
      ...
      <header className={`backdrop-blur-md border-b px-6 py-3.5 flex items-center justify-between sticky top-0 z-10 ${isDark ? 'bg-gray-800/80 border-gray-700' : 'bg-white/90 border-gray-200'}`}>
        ...
        <div className="flex items-center gap-3">
          {hasSeoData && (
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-gray-700/60 rounded-xl border border-gray-600/50 text-xs">
...
function CollingAIFlowAnim() {
  ...
  const STEPS = [
    {
      color: '#6366f1', label: 'Your Message', sub: 'Plain English',
      desc: 'Type any instruction — write a post, edit a page, update a meta description.',
    ...
    {
      color: '#a855f7', label: 'Claude AI', sub: 'Thinks & plans',
      desc: 'Claude reads your message, reasons through the best approach, and decides which WordPress REST API calls to make.',
    ...
    {
      color: '#3b82f6', label: 'WordPress API', sub: 'Action executed',
      desc: 'The WordPress REST API receives the call and creates, updates, or retrieves your content instantly.',
      icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" /></svg>,
    },
    {
      color: '#22c55e', label: 'Draft Saved', sub: 'Safe & ready',
      desc: 'Content lands as a draft in WordPress. Review it in your dashboard and publish whenever you\'re ready.',
  ...
  return (
    <div className="rounded-2xl border border-gray-700/50 overflow-hidden mb-4" style={{ background: 'rgba(17,24,39,0.6)' }}>
      <style>{`
        @keyframes packetMove { from { left: 0%; } to { left: 100%; } }
        @keyframes nodePulse { 0%,100% { box-shadow: 0 0 0 0 var(--nc); } 50% { box-shadow: 0 0 18px 4px var(--nc); } }
      `}</style>
          ...
          <React.Fragment key={i}>
            <div className="flex flex-col items-center gap-2 flex-1" style={{ opacity: active === i ? 1 : 0.32, transition: 'opacity 0.5s' }}>
              <div style={{
                ...
                background: active === i ? `${s.color}20` : '#1f2937',
                border: `1.5px solid ${active === i ? s.color : '#374151'}`,
                color: active === i ? s.color : '#4b5563',
                transform: active === i ? 'scale(1.12)' : 'scale(1)',
                transition: 'all 0.45s ease',
                '--nc': `${s.color}55`,
                animation: active === i ? 'nodePulse 1.6s ease-in-out infinite' : 'none',
              }}>
                {s.icon}
              </div>
                ...
                <div style={{ flex: 1, height: 2, background: '#1f2937', borderRadius: 4, overflow: 'hidden', position: 'relative' }}>
                  ...
                  {active === i && (
                    <div style={{
                      ...
                      animation: 'packetMove 2.4s linear forwards',
                    }} />
                  )}
                </div>
                {/* Arrowhead */}
                <svg width="6" height="10" viewBox="0 0 6 10" style={{ flexShrink: 0, marginLeft: 1 }}>
                  <path d="M0 0 L6 5 L0 10 Z" fill={active > i ? s.color : '#374151'} style={{ transition: 'fill 0.5s' }} />
                </svg>
              </div>
            )}
          </React.Fragment>
      ...
      <div style={{ padding: '8px 20px 12px', borderTop: '1px solid #1f293766', background: '#0f172a66' }}>
        <p style={{ fontSize: 11.5, color: '#9ca3af', textAlign: 'center', minHeight: 16 }}>
          {STEPS[active].desc}
        </p>
      </div>
    </div>
...
// ---------------------------------------------------------------------------
// Animation: SEO Bot pipeline
// ---------------------------------------------------------------------------

function SEOBotFlowAnim() {
  const [active, setActive] = useState(0);
  const [displayScore, setDisplayScore] = useState(28);

  const BEFORE_SCORE = 28;
  const AFTER_SCORE  = 86;
  ...
  // Animate score counter on last step
  useEffect(() => {
    if (active === 4) {
      let v = BEFORE_SCORE;
      const t = setInterval(() => {
        v += 4;
        if (v >= AFTER_SCORE) { setDisplayScore(AFTER_SCORE); clearInterval(t); }
        else setDisplayScore(v);
      }, 40);
  ...
  const STEPS = [
    {
      color: '#6366f1', label: 'Nightly Sweep', sub: '2:00 AM daily',
      desc: 'The SEO Bot scans every published post and page across all your connected sites.',
      icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>,
    },
    {
      color: '#ef4444', label: 'Score: 28', sub: 'Critical',
      desc: 'A post is found with a critical SEO score of 28. It\'s missing a focus keyword, meta title, and internal links.',
    ...
    {
      color: '#f59e0b', label: 'Job Queued', sub: 'Priority 2 — medium',
      desc: 'The post is added to the SEO job queue at medium priority. It will be processed in the next 30-minute cycle.',
    ...
    {
      color: '#a855f7', label: 'Claude Optimizes', sub: 'AI writes metadata',
      desc: 'Claude generates a focus keyword, meta title (50–60 chars), meta description (140–160 chars), and suggests internal links.',
      icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" /></svg>,
    },
    {
      color: '#22c55e', label: 'Score: 86 ✓', sub: 'Good — complete',
      desc: 'The post is re-scored after optimisation. Score jumped from 28 → 86. The job is marked complete.',
      icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
    },
  ];
  ...
  return (
    <div className="rounded-2xl border border-gray-700/50 overflow-hidden mb-4" style={{ background: 'rgba(17,24,39,0.6)' }}>
      <style>{`
        @keyframes seoPacket { from { left: 0%; } to { left: 100%; } }
        @keyframes seoNodePulse { 0%,100% { box-shadow: 0 0 0 0 var(--sc); } 50% { box-shadow: 0 0 18px 4px var(--sc); } }
      `}</style>

      {/* Live score ring — top right */}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <svg width="44" height="44" viewBox="0 0 44 44" style={{ transform: 'rotate(-90deg)' }}>
            ...
            <circle cx="22" cy="22" r={r} fill="none" stroke={ringColor} strokeWidth="4"
              ...
              style={{ transition: 'stroke-dasharray 0.2s, stroke 0.4s' }} />
          </svg>
          <div>
            <p style={{ fontSize: 20, fontWeight: 800, color: ringColor, lineHeight: 1, transition: 'color 0.4s' }}>{displayScore}</p>
            <p style={{ fontSize: 10, color: '#4b5563' }}>/ 100</p>
          </div>
        </div>
      </div>
          ...
          <React.Fragment key={i}>
            <div className="flex flex-col items-center gap-2 flex-1" style={{ opacity: active === i ? 1 : 0.3, transition: 'opacity 0.5s' }}>
              <div style={{
                ...
                '--sc': `${s.color}55`,
                animation: active === i ? 'seoNodePulse 1.6s ease-in-out infinite' : 'none',
              }}>
                {s.icon}
              </div>
                ...
                <div style={{ flex: 1, height: 2, background: '#1f2937', borderRadius: 4, overflow: 'hidden', position: 'relative' }}>
                  ...
                  {active === i && (
                    <div style={{
                      position: 'absolute', top: '50%', transform: 'translateY(-50%)',
                      width: 6, height: 6, borderRadius: '50%', background: s.color,
                      animation: 'seoPacket 2.2s linear forwards',
                    }} />
                  )}
                </div>
                <svg width="5" height="9" viewBox="0 0 5 9" style={{ flexShrink: 0, marginLeft: 1 }}>
                  <path d="M0 0 L5 4.5 L0 9 Z" fill={active > i ? s.color : '#374151'} style={{ transition: 'fill 0.5s' }} />
                </svg>
              </div>
            )}
          </React.Fragment>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-10 pb-8">
      ...
      <Section title="2. How to Add a Site">
        ...
        {/* Site ownership rule — shown inline here as it's directly relevant */}
      ...
      <Section title="4. How the SEO AI Works">
        ...
        <div className="bg-gray-800/60 border border-gray-700 rounded-2xl p-6 space-y-1">
          ...
          <InfoCard number="4" title="Runs 24/7 on the backend server">
            <p>The SEO Bot runs on the backend even when the desktop app is closed — every 5 minutes for urgent jobs, every 30 minutes for medium-priority, and a full nightly sweep at 2 AM.</p>
          </InfoCard>
        </div>
      </Section>
...
function SettingsTab({ user, setUser }) {
  ...
  const handleSaveName = async () => {
    ...
    setNameSaving(true);
    setNameMsg(null);
    const result = await window.electronAPI.invoke('auth:update-profile', { name: name.trim() });
    setNameSaving(false);
    if (result.error) {
      setNameMsg({ type: 'err', text: result.error });
    } else {
  ...
  const handleSavePassword = async () => {
    ...
    setPwSaving(true);
    setPwMsg(null);
    const result = await window.electronAPI.invoke('auth:update-password', { currentPassword: currentPw, newPassword: newPw });
    setPwSaving(false);
    if (result.error) {
    ...
    <div className="max-w-lg mx-auto space-y-6 pb-8">
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-2xl overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          ...
          {nameMsg && (
            <p className={`text-xs ${nameMsg.type === 'ok' ? 'text-emerald-400' : 'text-red-400'}`}>{nameMsg.text}</p>
          )}
          <button
