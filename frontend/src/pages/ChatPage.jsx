...
export default function ChatPage({ onBack }) {
  const { activeSite, chatHistory, appendMessage, setChatHistory, isLoading, setLoading } = useAppStore();
  ...
  const detectSeo = async () => {
    ...
    try {
      const res = await client.post('/chat/detect-seo-plugin', {
      ...
      if (res.data.plugin) setDetectedSeoPlugin(res.data.plugin);
  ...
  const isWordPressDeletionQuery = (message) => {
    ...
    return deletionKeywords.some((k) => lower.includes(k)) && wordpressKeywords.some((k) => lower.includes(k));
  };
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      if (result.error) {
        ...
        if (result.thinking) {
          setThinkingMap((prev) => ({ ...prev, [assistantIndex]: result.thinking }));
        }
        if (result.actionResult) {
          setActionResults((prev) => ({ ...prev, [assistantIndex]: result.actionResult }));
        }
      }
    } catch (err) {
      appendMessage(siteId, {
  ...
  const handlePaste = (e) => {
    ...
    if (fileItems.length > 0) {
      e.preventDefault();
      fileItems.forEach((item) => { const file = item.getAsFile(); if (file) processFiles([file]); });
    }
  };
      ...
      <header className="bg-gray-800/80 backdrop-blur-md border-b border-gray-700 px-4 py-3 flex items-center gap-3 flex-shrink-0">
        ...
        <div className="flex-1 flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h2 className="text-white font-semibold text-sm truncate">{siteLabel}</h2>
            <p className="text-gray-500 text-xs font-mono truncate">{getDomainLabel(siteUrl)}</p>
          </div>
          {seoPluginLabel && (
