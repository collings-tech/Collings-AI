            ...
            {loading
              ...
              ? posts.map((p) => (
                  <tr key={`${p.postId}-${p.createdAt}`} className="border-t border-gray-700/50 hover:bg-gray-700/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center"><span className="text-green-400 font-semibold">+{p.improvement}</span></td>
                    <td className="px-4 py-3 text-right text-gray-500">{formatDate(p.createdAt)}</td>
                  </tr>
                ))
              : (
                <tr>
                  <td colSpan={5} className="px-5 py-8 text-center text-gray-500 text-sm">No improvements recorded this month</td>
                </tr>
              )}
          </tbody>
        </table>
